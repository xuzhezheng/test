...
def first_stage_regression(y='ESG_Uncertainy_others2',x='senti',cvs=['Inst_invest','Inde_director','Ten_share','Duality','Analyst','Assets','Debt','ROA','Fixed','Bktomk']):
    ...
    CS = pd.read_csv("../data/Heterogeneity/CS_Yearmean.csv")#2015-2020
    CS = CS[['Scode', 'CompanySize']]

    mean_val = CS['CompanySize'].mean()  # 计算平均值

    CS['CS_Type'] = (CS['CompanySize'] > mean_val).astype(int)  # 大于平均值为1，否则为0
    CS = CS[['Scode', 'CS_Type']]

    # Ind = Ind[['Scode', 'Year', 'Pollution']]
    #Polluton = 0非污染, Pollution = 1 污染
    CS = CS.dropna()
    CS['Scode'] = CS['Scode'].astype(int)
    # Ind['Year'] = Ind['Year'].astype(int)
    CS_colname = CS.columns[-1]



    merged_df = merge_df(y,x,cv,CS)

    merged_df['interaction'] = merged_df[x_colname] * merged_df[CS_colname]
    ...
    ## 小企业（<=平均值）
    Sm_df = df[df[CS_colname] == 0]
    print("=====" * 10, "Small_Company", "=====" * 10)

    formula = 'ESG_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'

    model_stage1 = smf.ols(formula=formula, data=Sm_df).fit(cov_type='cluster', cov_kwds={'groups': Sm_df['indcd']})

    print("=====" * 10, "model_stage1", "=====" * 10)
    print(model_stage1.summary())

    formula = 'E_Uncert ~ 1 + ' + x_colname  + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage2 = smf.ols(formula=formula, data=Sm_df).fit(cov_type='cluster', cov_kwds={'groups': Sm_df['indcd']})
    print("=====" * 10, "model_stage2", "=====" * 10)
    print(model_stage2.summary())

    formula = 'S_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage3 = smf.ols(formula=formula, data=Sm_df).fit(cov_type='cluster', cov_kwds={'groups': Sm_df['indcd']})
    print("=====" * 10, "model_stage3", "=====" * 10)
    print(model_stage3.summary())

    formula = 'G_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage4 = smf.ols(formula=formula, data=Sm_df).fit(cov_type='cluster', cov_kwds={'groups': Sm_df['indcd']})
    ...
    ## 大企业组（<=平均值）
    Big_df = df[df[CS_colname] == 1]
    print("=====" * 10, "big_company", "=====" * 10)

    formula = 'ESG_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage1 = smf.ols(formula=formula, data=Big_df).fit(cov_type='cluster', cov_kwds={'groups': Big_df['indcd']})

    print("=====" * 10, "model_stage1", "=====" * 10)
    print(model_stage1.summary())

    formula = 'E_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage2 = smf.ols(formula=formula, data=Big_df).fit(cov_type='cluster', cov_kwds={'groups': Big_df['indcd']})
    print("=====" * 10, "model_stage2", "=====" * 10)
    print(model_stage2.summary())

    formula = 'S_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage3 = smf.ols(formula=formula, data=Big_df).fit(cov_type='cluster', cov_kwds={'groups': Big_df['indcd']})
    print("=====" * 10, "model_stage3", "=====" * 10)
    print(model_stage3.summary())

    formula = 'G_Uncert ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    model_stage4 = smf.ols(formula=formula, data=Big_df).fit(cov_type='cluster', cov_kwds={'groups': Big_df['indcd']})
    print("=====" * 10, "model_stage4", "=====" * 10)
    print(model_stage4.summary())
