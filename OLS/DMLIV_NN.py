...
def DeepIV_regression(y='ESG_Uncertainy_others2',x='senti',ivs = 'network_mean' ,cvs=['Inst_invest','Inde_director','Ten_share','Duality','Analyst','Assets','Debt','ROA','Fixed','Bktomk']):
    ...
    model_y_xw=RandomForestRegressor(random_state=42),
        model_t_xwz=MLPClassifier(hidden_layer_sizes=(64,32),
                      activation='tanh',
                      solver='adam',
                      max_iter=500,
                      random_state=42),
        model_t_xw=MLPClassifier(hidden_layer_sizes=(64,32),
                      activation='tanh',
                      solver='adam',
                      max_iter=500,
                      random_state=42),
    ...
    with open("../results/DMLNN_effect_inference_result_x更正.txt", "a", encoding="utf-8") as f:
        f.write("**" * 40)
        f.write("\n")
        for i in range(2, 20):
