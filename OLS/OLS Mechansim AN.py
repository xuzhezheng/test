...
def first_stage_regression(y='ESG_Uncertainy_others2',x='senti',an='Analyst',cvs=['Inst_invest','Inde_director','Ten_share','Duality','Analyst','Assets','Debt','ROA','Fixed','Bktomk']):
    ...
    #     sr['Negratio'] = sr['Negative'] / sr['Positive']
    ...
    merged_df['interaction'] = merged_df[x_colname] * merged_df[an]
    ...
    #Step1 : X->M
    formula = an + ' ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    ...
    #交乘项
    formula = 'ESG_Uncert ~ 1 + ' + x_colname + " + " + an + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage1 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    ...
    formula = 'E_Uncert ~ 1 + ' + x_colname + " + " + an + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage2 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    print("=====" * 10, "model_stage2", "=====" * 10)
    print(model_stage2.summary())

    formula = 'S_Uncert ~ 1 + ' + x_colname + " + " + an + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage3 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    print("=====" * 10, "model_stage3", "=====" * 10)
    print(model_stage3.summary())

    formula = 'G_Uncert ~ 1 + ' + x_colname + " + " + an + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage4 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    print("=====" * 10, "model_stage4", "=====" * 10)
    print(model_stage4.summary())
...
first_stage_regression(y='ESG_Uncertainy_others2',x='newsnum',an='Analyst',cvs=['Boardsize','Age','Inst_invest','Inde_director',
