...
def DeepIV_regression(y='ESG_Uncertainy_others2',x='senti',ivs = 'network_mean' ,cvs=['Inst_invest','Inde_director','Ten_share','Duality','Analyst','Assets','Debt','ROA','Fixed','Bktomk']):
    ...
    Z = df[iv_list]
    print(Z)
    T = winsorize(np.log1p(df[x_colname]), limits=[0.01, 0.01])
    ...
    print(est.ate_inference(X))
    with open("../results/DMLIVTreeC_effect_inference_result_x更正.txt", "a", encoding="utf-8") as f:
        f.write("**" * 40)
        f.write("\n")

        effect = est.const_marginal_ate_inference(X=X)
        f.write(str(effect))
