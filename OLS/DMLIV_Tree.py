...
def DeepIV_regression(y='ESG_Uncertainy_others2',x='senti',ivs = 'network_mean' ,cvs=['Inst_invest','Inde_director','Ten_share','Duality','Analyst','Assets','Debt','ROA','Fixed','Bktomk']):
    ...
    model_y_xw=RandomForestRegressor(random_state=42),
        model_t_xwz=RandomForestClassifier(random_state=42),
        model_t_xw=RandomForestClassifier(random_state=42),
    ...
    # print(est.const_marginal_ate_inference(X=X))

    with open("../results/DMLIVTree_effect_inference_result_x更正.txt", "a", encoding="utf-8") as f:
        f.write("**" * 40)
        f.write("\n")
        for i in range(2, 20):
            effect = est.ate_inference(X, T0=i-1, T1=i)
            f.write(str(effect))
