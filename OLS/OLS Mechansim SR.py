...
def first_stage_regression(y='ESG_Uncertainy_others2',x='senti',sr='neg',cvs=['Inst_invest','Inde_director','Ten_share','Duality','Analyst','Assets','Debt','ROA','Fixed','Bktomk']):
    ...
    print("=====" * 10, "将{}作为缓解信息不对称的代理指标".format(sr), "=====" * 10)
    ...
    if sr == 'neg':
        sr = pd.read_csv("../data/Mechanism/Social regulation.csv")
        sr = sr[['Scode','Year','Negative']]
    elif sr == 'negratio':
        sr = pd.read_csv("../data/Mechanism/Social regulation.csv")
        sr = sr[['Scode', 'Year', 'Positive','Negative','Post','Neutral']]
        sr['Negratio'] = sr['Negative'] / sr['Post']
        sr = sr[['Scode','Year','Negratio']]
    sr = sr.dropna()
    sr['Scode'] = sr['Scode'].astype(int)
    sr['Year'] = sr['Year'].astype(int)
    sr_colname = sr.columns[-1]


    merged_df = merge_df(y,x,cv,sr)

    merged_df['interaction'] = merged_df[x_colname] * merged_df[sr_colname]
    ...
    #Step1 : X->M
    formula = sr_colname + ' ~ 1 + ' + x_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    ...
    #Step2 : X + M -> Y(ESG,E,S,G)

    # #非交乘项
    # formula = 'ESG_Uncert ~ 1 + ' + x_colname + " + " + sr_colname  + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    ...
    #
    # formula = 'E_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    ...
    #
    # formula = 'S_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    ...
    #
    # formula = 'G_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + " + ".join(cvs) + ' + C(Year) + C(indcd)'
    ...
    #交乘项
    formula = 'ESG_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage1 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    ...
    formula = 'E_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage2 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    print("=====" * 10, "model_stage2", "=====" * 10)
    print(model_stage2.summary())

    formula = 'S_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage3 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    print("=====" * 10, "model_stage3", "=====" * 10)
    print(model_stage3.summary())

    formula = 'G_Uncert ~ 1 + ' + x_colname + " + " + sr_colname + " + " + "interaction" + " + " + " + ".join(
        cvs) + ' + C(Year) + C(indcd)'
    model_stage4 = smf.ols(formula=formula, data=df).fit(cov_type='cluster', cov_kwds={'groups': df['indcd']})
    print("=====" * 10, "model_stage4", "=====" * 10)
    print(model_stage4.summary())
...
first_stage_regression(y='ESG_Uncertainy_others2',x='newsnum',sr='neg',cvs=['Boardsize','Age','Inst_invest','Inde_director',
                                        'Ten_share','Duality','ROA','Fixed'])

first_stage_regression(y='ESG_Uncertainy_others2',x='newsnum',sr='negratio',cvs=['Boardsize','Age','Inst_invest','Inde_director',
                                        'Ten_share','Duality','ROA','Fixed'])
