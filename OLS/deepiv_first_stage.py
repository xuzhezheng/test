"""Train only the first stage of DeepIV."""
...
def deepiv_first_stage(
    ...
    df = df[["Scode", "Year", x_col, *iv_list, *cvs]].dropna()

    X = df[cvs].values
    Z = df[iv_list].values

    model = build_treatment_model(Z.shape[1] + X.shape[1])
    fit_opts = {"epochs": 30, "validation_split": 0.0}

    est = DeepIV(
        n_components=10,
        m=lambda z, x: model(keras.layers.concatenate([z, x])),
        h=lambda t, x: model(keras.layers.concatenate([t, x])),
        n_samples=1,
        first_stage_options=fit_opts,
        second_stage_options=fit_opts,
    )
    est.fit(np.zeros(len(Z)), np.zeros(len(Z)), X=X, Z=Z)
    print("First stage trained.")


if __name__ == "__main__":
    deepiv_first_stage(x="newsnum", ivs="paper_count")
